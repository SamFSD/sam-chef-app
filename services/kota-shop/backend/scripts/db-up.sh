...
if docker exec -u postgres db-local psql postgres postgres -c "CREATE DATABASE wesbank;"; then
    echo "created database because did not exist"

    # if the database was created, then wait for the db to be ready
    timeout 30s bash -c "until docker exec db-local psql wesbank postgres -c '' ; do echo '⌛'; sleep 3 ; done"; echo -e \\a
    echo "created database ready for connections"
fi

# execute the alembic up grade inside the container
docker exec -u fish -w /app/db proj_wesbank_backend alembic upgrade head
