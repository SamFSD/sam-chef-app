...
docker exec -u root -w /app/db proj_wesbank_backend alembic revision --autogenerate

# # create the revision with a version message
# docker exec -u fish -w /app/db proj_wesbank_backend alembic revision --autogenerate -m "message"

# # gerenate an sql file before upgrading the database to make sure the sql is correct
# docker exec -u root -w /app/db proj_wesbank_backend alembic upgrade head --sql > services/_api/src/db/sql/base.sql
