...
import { MonthpickerService } from './monthpicker/monthpicker.service';
...
export class SmallFormService {
  ...
  canViewUsersLogs: boolean = false;
    ...
    if (cachedPermissions) {
      ...
      return of(this.userPermissions).pipe(
        tap(() => {
          ...
          // this.checkIfUserCanSeeUserReport()
        })
      );
    }
      ...
      switchMap((profile) => {
        ...
        return this.getUserPermissions().pipe(
          tap((permissions) => {
            ...
            // set user permissions that can view users log-in logs
            this.canViewUsersLogs = permissions[0].can_view_users_logs;
  ...
  get divisionArray() {
    return this.form.get('division') as FormArray;
  }

  setBranchesOnDivisionChange() {
    this.form.controls['division'].valueChanges.subscribe((clickedDivisions: string[]) => {
      console.log("Clicked division: ", clickedDivisions);
      let allowedBranches: any[] = [];
      this.considerAllDivisions = clickedDivisions.includes('full_fleet');

      this.userPermissions.forEach((div: any) => {
        console.log("Division", div);
        if (this.considerAllDivisions || clickedDivisions.includes(div.division)) {
          allowedBranches.push(...div.branches.map((branch: any) => {
            console.log("Our checked branch: ", branch);
            return { branch: branch };
          }));
        }
      });
      
      console.log("Current Allowed Branches: ", this.allowedBranches);
      this.allowedBranches = Array.from(new Set(allowedBranches));
      console.log("Allowed Branches: ", this.allowedBranches);
      this.canSubmit = false;

      if (this.allowedBranches.length > 0) {
        this.form.controls['branch_single'].setValue(this.allowedBranches[0].branch);
      }
      ...
      (selectedReport) => {
        ...
        switch (selectedReport) {
          ...
          case 'Fleet Card':
            ...
            this.showSupplierSelect = false;
            break;
          // log-in count and time report
          case 'LoggedIn Users Count And Time':
            // Condition to check if the user can view the report
            this.showDivisionSelect = false;
            this.showBranchSelect = false;
            this.showVehTypeSelect = false;
            this.showVehicleSelect = false;
            this.showSupplierSelect = false;
            this.showComponentSelect = false;
            break;
  ...
  //check if user can see loggedin user count and time method to push to report types
  checkIfUserCanSeeUserReport() {
    if (!this.canViewUsersLogs) {
      this.reportTypes.push(this.userReportType);
    }
  }

  userReportType = {
    reportTitle: 'LoggedIn Users Count And Time',
    reportDescription: 'LoggedIn Users Count And Time Report',
    selectableFields: [],
  };
