...
@Component({
  selector: 'app-pav-supplier-summary',
  templateUrl: './pav-supplier-summary.component.html',
  styleUrls: ['./pav-supplier-summary.component.scss'],
})
export class PavSupplierSummaryComponent {
  constructor(
  ...
  callApi() {
    this.isLoading = true;
    this.api
      .getPavSupplierV0GetPavSuppliersPost(this.smallForm.getFormValues())
      .pipe(takeUntil(this.onDestroy))
      .subscribe({
        next: (res: any) => {
  ...
  generateGraph() {
    if (!this.graphData || this.graphData.length === 0) {
      ...
      const data = this.graphData.map(
        (supItem: any) => supItem.serviceprovider
      );
